...
(function() {
	...
	CKEDITOR.env || (CKEDITOR.env = function() {
		...
		if (f <= 11E3) a.cssClass = a.cssClass + " cke_browser_gecko19";
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			copyAttributes: function(a, b) {
				for (var c = this.$.attributes, b = b || {}, d = 0; d < c.length; d++) {
					...
					if (g.specified || CKEDITOR.env.ie && g.nodeValue && k == "value") {
			...
			function l(b, c) {
				return function(l) {
					if (l.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						if (!t && CKEDITOR.env.ie && (document.documentMode > 7 || l.name in CKEDITOR.dtd.tr || l.name in CKEDITOR.dtd.$listItem)) e = false;
						else {
							e = a(l);
					...
					if (!this.is("textarea")) {
						...
						this.attachListener(a, "key", function(b) {
							...
							if (c in {
								...
								if (f.blockLimit && f.blockLimit.is("td") &&
	...
	(function() {
		function n(a) {
			...
			if (c.config.disableObjectResizing) try {
				...
				if (c.window.$.innerHeight > this.$.offsetHeight) {
					var d = c.createRange();
